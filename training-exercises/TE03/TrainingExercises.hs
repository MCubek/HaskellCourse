...
te311 (_, 'H':rest, _) = length rest > 5
te311 _ = False
...
te312 user@(usn, _, lit) = (usn, te311 user, lit)
...
te321 :: (Fractional a) => [Double] -> a
te321 xs = if length list == 0 then error "Error" else avg
  where
    list = drop 1 (take (length xs - 1) xs)
    avg = realToFrac (sum list) / genericLength list
...
te322 :: String -> String
te322 [] = error "Error"
te322 string = "The string has " ++ postfix
    where postfix = case string of
            [_]    -> "one character"
            [_, _] -> "two characters"
            _:_    -> "many characters"
...
te323 :: (Fractional b, Integral a) => [a] -> (b, b, b)
te323 xs = (q1, q2, q3)
  where
    q1 = median (take len sorted)

    q2 = median xs

    q3 = median (take len (reverse sorted))
