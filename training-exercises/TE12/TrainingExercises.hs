...
stockIsFromFarmer :: String -> FruitStock -> Bool
stockIsFromFarmer farmerIdToCheck (FruitStock _ _ (Just (Farmer _ _ (Just farmerIdStock)))) = farmerIdToCheck == farmerIdStock
stockIsFromFarmer _ _ = False

te1211 :: String -> Int
te1211 id = sum $ map amount $ filter (stockIsFromFarmer id) company_stock

te1211m :: String -> Int
te1211m id = sum $ mapMaybe (mapMatchingStockToAmount id) company_stock

mapMatchingStockToAmount :: String -> FruitStock -> Maybe Int
mapMatchingStockToAmount id stock = if (farmer stock >>= farmerId) == Just id then Just (amount stock) else Nothing
...
getIds :: Fruit -> [String]
getIds givenFruit = mapMaybe (farmer >=> farmerId) $ filter ((== givenFruit) . fruit) company_stock

te1212 :: IO ()
te1212 = do
    putStrLn "Enter the name of the fruit:"
    fruitName <- getLine
    mapM_ putStrLn $ getIds (read fruitName)
...
te1223 :: Int -> Int
te1223 = stateSM $ multiplyBy2 >> multiplyBy2 >> add3 
