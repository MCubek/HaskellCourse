...
te915 pairs f g = map (\(Pair x y) -> Pair (f x) (g y)) pairs
...
te942 Nil = 0
te942 tree = te942 (left tree) + value tree + te942 (right tree)
...
te943 Nil = Nothing
te943 tree =
  maximum [currentValue, maxLeft, maxRight]
  where
    currentValue = Just $ value tree
    maxLeft = te943 $ left tree
    maxRight = te943 $ right tree
